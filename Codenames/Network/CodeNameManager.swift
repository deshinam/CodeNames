...
class CodeNameManager {
    ...
    func checkGame(codeName: String, closure: @escaping (Bool)->()) {
        reference?.child("dictionary").child("busy").child(codeName).observeSingleEvent(of: DataEventType.value, with: {snapshot in
            if (snapshot.value == nil) || (Date().timeIntervalSince1970 - (snapshot.value as? Double ?? 0) >= 60)  {
                closure(false)
            } else {
                closure(true)
            }
